            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="mt-4 text-sm text-gray-600 space-y-1">
                    ...
                    @if(!empty($oferta->fecha_publicacion))
                        <div>Publicación: <strong>{{ \Carbon\Carbon::parse($oferta->fecha_publicacion)->format('d/m/Y') }}</strong></div>
                    @endif
                    @if(!empty($oferta->publicar_hasta))
                        <div>Visible hasta: <strong>{{ \Carbon\Carbon::parse($oferta->publicar_hasta)->format('d/m/Y') }}</strong></div>
                    @endif
                    @if(!empty($oferta->fecha_incorporacion))
                        <div>Incorporación: <strong>{{ \Carbon\Carbon::parse($oferta->fecha_incorporacion)->format('d/m/Y') }}</strong></div>
                    @endif
                </div>
